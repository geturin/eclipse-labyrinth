...
test('basic attack regenerates exactly 1 MP without a bonus weapon',()=>{const r=ready('ninja');r.party[0].weapon.effect='none';r.party[0].mp=1;deal(r);assert.equal(r.party[0].mp,2);});
test('guard restores MP and remains active until next turn',()=>{const r=ready();r.party[0].mp=0;assert.equal(act(r,'guard').ok,true);assert.equal(r.party[0].mp,3);assert.equal(r.party[0].guard,true);});
...
test('basic attacks no longer build a free skill chain',()=>{const r=ready();deal(r);const second=activeHero(r);assert.notEqual(second.id,r.party[0].id);assert.equal(act(r,'attack',r.battle.enemies[0].id).ok,true);assert.equal(r.battle.chain,0);});
test('reaver drains health and blood pact cannot self-kill',()=>{const r=ready('reaver');r.party[0].hp=20;deal(r,'rend');assert.ok(r.party[0].hp>20);const s=ready('reaver');s.party[0].hp=1;s.party[0].mp=0;act(s,'bloodpact');assert.equal(s.party[0].hp,1);assert.equal(s.party[0].mp,8);assert.ok(s.party[0].status.fury);});
...
test('revive brings a fallen ally back and heal cannot target an enemy',()=>{const r=ready('shrine');r.party[0].skills.push('revive');r.party[1].hp=0;const mp=r.party[0].mp;assert.equal(act(r,'revive','enemy-0').ok,false);assert.equal(r.party[0].mp,mp);assert.equal(act(r,'revive',r.party[1].id).ok,true);assert.ok(r.party[1].hp>0);});
...
test('haste is single-target and ranks reduce MP, not infinite duration',()=>{for(let rank=0;rank<=2;rank++){const r=ready('chrono');r.party[0].ranks.haste=rank;const speed=heroStats(r.party[0]).spd;assert.ok(act(r,'haste',r.party[0].id).ok);assert.equal(r.party[0].status.haste.turns,3);assert.equal(heroStats(r.party[0]).spd,speed*1.5);assert.equal(r.party[1].status.haste,undefined);assert.equal(skillCost(r.party[0],'haste'),6-rank);}});
...
test('weapon: mana restores 1 extra MP after a guard',()=>{const r=ready();r.party[0].weapon.effect='mana';r.party[0].mp=0;act(r,'guard');assert.equal(r.party[0].mp,4);});
...
test('weapon: soulsteal heals and restores MP on kill',()=>{const r=ready();r.battle.enemies.push({...copy(r.battle.enemies[0]),id:'enemy-9'});r.party[0].weapon.effect='soulsteal';r.party[0].hp=10;r.party[0].mp=0;r.battle.enemies[0].hp=1;deal(r);assert.ok(r.party[0].hp>20);assert.equal(r.party[0].mp,7);});
...
test('whole party defeat ends the run',()=>{const r=createRun(['knight'],'loss');r.party[0].spd=100;startBattle(r);for(const foe of r.battle.enemies){foe.atk=10000;foe.mag=10000;foe.intent=['attack'];foe.plannedIntent='attack';}r.party[0].hp=1;act(r,'guard');assert.equal(r.phase,'ended');assert.equal(r.ending,'defeat');});
