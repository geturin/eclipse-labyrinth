...
def settle(page):page.wait_for_function("!document.querySelector('#combat-skip')",timeout=15000)
def skill(page,id):
    if id in ['tonic','ether','salt'] and not page.locator(f'[data-skill="{id}"]').count():
        page.locator('[data-action="toggle-supplies"]').click()
    page.locator(f'[data-action="skill"][data-skill="{id}"]').click()
def main():
    with sync_playwright() as p:
        browser=p.chromium.launch(executable_path=os.environ.get('CHROMIUM_PATH','/usr/bin/chromium'),args=['--no-sandbox'])
        page=launch(browser)
        check('UI starts in party preparation with no MP bar',page.locator('.party-strip .party-card').count()==3 and 'MP' not in page.locator('#party-strip').inner_text())
        ...
        check('Used skill button has a disabled cooldown rather than an MP cost',page.locator('[data-skill="cleave"]').is_disabled() and '冷却 3' in page.locator('[data-skill="cleave"]').inner_text())
        page.locator('[data-action="party"][data-id="hero-1"]').click();skill(page,'seal');settle(page)
